...
- **CẬP NHẬT 10/7 (hack day) — ĐÃ BUILD XONG, khác spec 05 gốc theo hướng tốt hơn:** thay vì mock cả
  ...
  số liệu detour (route 2 lần: baseline vs có điểm dừng). **Còn mô phỏng: sự kiện xe qua trạm + đồng
  hồ chuyến đi (giờ bữa ăn)**, luôn gắn nhãn "MÔ PHỎNG/SIMULATED" trên UI. Dữ liệu mock cũ ở `mock/`
  không còn được đọc. Chi tiết đầy đủ + cách chạy: `module5/README.md`.
- **CẬP NHẬT 10/7 (bản 2) — meal-window gating + sửa câu chuyện differentiator:** bản đầu đẩy gợi ý mỗi
  lần qua trạm bất kể giờ (phiền + không thật khác Google Maps, vì GPS geofence GG Maps xấp xỉ được
  "biết bạn gần trạm"). Nay endpoint `/v1/route/rest-stops` nhận `?atHour=` và **chỉ đẩy trong khung
  giờ bữa** (sáng/trưa/tối/khuya); ngoài khung → im lặng (`push:false`, badge only). **Differentiator
  bán ra:** route-aware (quán phía trước + detour thật) + structured amenity ô tô/gia đình (GG Maps VN
  thiếu) + đẩy chủ động vì tài xế đang lái — **KHÔNG** phải "bí mật giao dịch VETC". Đã cân nhắc & loại
  hướng trạm sạc EV (zero data/API, phải mock 100%, đổi persona).
