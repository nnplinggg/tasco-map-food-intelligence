...
**US-5 — Tài xế đang lái xe trên cao tốc, vừa qua trạm thu phí VETC** *(Module 5 — differentiator, ĐÃ BUILD)*

> Là một tài xế đang chở gia đình từ Hà Nội đi Hạ Long, xe vừa đi qua trạm thu phí không dừng, tôi đói
> và muốn dừng ăn tối mà không phải cầm điện thoại tra cứu trong lúc lái xe.
...
### Module 5 — Gợi ý điểm dừng dọc hành trình (VETC Route Corridor Demo) *(stretch — 0 điểm benchmark, cắt đầu tiên nếu bí giờ — ĐÃ BUILD XONG)*
...
- **Vì sao đúng:** không xây route engine mới — gọi thẳng API production thật của Tasco (Valhalla +
  Pelias) cho tuyến đường, trạm thu phí, quán ăn, và detour; tái dùng 100% engine lọc/xếp hạng của
  Module 1 (`module5/lib/module1_engine.js` — hiện thực đúng công thức spec 01 §4, **Module 1 nên
  import file này thay vì viết lại**, để 2 module luôn dùng chung 1 công thức xếp hạng). Chỉ 1 thứ mô
  phỏng: thời điểm xe qua trạm. Dữ liệu Module 5 **cách ly hoàn toàn** khỏi 30 quán benchmark — có test
  tự động quét mọi file trong `module5/` để đảm bảo không file nào đọc 5 CSV gốc.
- **Cập nhật 10/7 so với kế hoạch gốc:** spec 05 ban đầu định mock cả trạm lẫn quán (lo ngại route
  engine thật tốn 4-6h). Thực tế API production đã sẵn sàng và miễn phí gọi trong ngày hack, nên build
  bằng dữ liệu thật rẻ hơn dự tính — không còn lý do giữ phần mock ngoài sự kiện qua trạm.
- **Ăn điểm:** 0/15 câu benchmark — giá trị nằm ở phần trình bày business/kiến trúc, không phải eval
  tự động.
- **Chạy demo:** `node module5/server.js` → `http://localhost:8790` (replay từ cache, không cần mạng
  lúc demo). Chi tiết đầy đủ (REAL vs MOCKED, cách tính detour): `module5/README.md`. Lý do quyết định
  gốc: `05-route-corridor-demo-spec.md`.
...
- DTO chung (PlaceResult, ErrorResponse) đã cố định theo API doc — xem mục 6 của `CLAUDE.md`. Ai thêm
  ...
  danh sách search.
- **Riêng Module 5:** sống trọn trong `module5/`, đọc dữ liệu qua API production thật (không phải CSV
  gốc) + cache riêng ở `module5/cache/`. **Tuyệt đối không được merge** quán/trạm của Module 5 vào bảng
  30 quán dùng cho `/v1/search` chính — nếu lẫn vào sẽ làm sai mọi ground truth đã đếm tay của Module
  1/3/4 (vd "8 quán có Bún chả"). Đã có test tự động (`module5/test/run_tests.js`) quét toàn bộ source
  Module 5 để đảm bảo không file nào đọc 5 CSV benchmark. Thư mục `mock/` cũ không còn dùng.
